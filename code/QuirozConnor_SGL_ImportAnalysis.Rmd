...
fishbase_presences <- taxa_presence_or_absence(data = "fishbase")
sealifebase_presences <- taxa_presence_or_absence(data = "sealifebase")
...
only_sealifebase <- sealifebase_presences %>%
  filter(source == "sealifebase")

# Get indexes where sealifebase found presences or absences
sealifebase_presence_absences_indexes <- which(sealifebase_presences$presence_or_absence %in% c(0,1))

# Select fishbase + unlinked NA data (excludes sealifebase)
fishbase_minus_sealifebase <- fishbase_presences[-sealifebase_presence_absences_indexes,]

# Merce fishbase / sealifebase presence data - this will be the data that we use to interpolate higher taxonomic resolutions!
merged_presence_absence_data <- rbind(fishbase_minus_sealifebase, only_sealifebase)
