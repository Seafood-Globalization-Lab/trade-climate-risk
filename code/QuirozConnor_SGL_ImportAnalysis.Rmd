...
```{r}
future_species_codes <- species_names %>%
  rename(sciname = "taxon_name") %>%
  mutate(sciname = tolower(sciname))

sciname_test <- left_join(species_names_renamed, sciname, by = "sciname")
consumption_sciname_test <- left_join(consumption, future_species_codes, by = "sciname")

length(unique(consumption_sciname_test$taxon_key))
length(unique(consumption$sciname))
length(unique(species_names_renamed$sciname))

consumption_sciname_test

length(future_species_codes$sciname)
sum(str_detect(future_species_codes$sciname, pattern = "\ "), na.rm = TRUE)
```
...
consumption_sciname <- consumption_sciname %>% mutate(
    region = case_when(
...
# # TRIAL REMOVE AFTER USE
# for (i in names(consumption_sciname)[10:14]) {
#   con_actual_group <- consumption_sciname %>%
#     filter(!is.na(i)) %>%
#     group_by(producer_iso3c) %>%
#     summarize(!!paste0("prop_actual_", i) := sum(!is.na(get(i))) / n()) %>%
...
#   # Join prop of each phylogenetic group to already joined consumption dataset
#   con_joined <-
#     left_join(con_joined, con_actual_group, by = "producer_iso3c")
#   
# }
...
(ssp_projections <- consumption_sciname %>%
...
test_data[test_data$year == 2100 & test_data$percent_change_catch > 100, ]
