...
pa_excluding_artis %>% distinct(genus) %>% pull(genus)
```
...
merged_presence_absence_data_country <- merged_presence_absence_data_country %>%
  mutate(species = case_when(str_detect(sciname_hs_modified, " ") ~ sapply(str_split(sciname_hs_modified, " "), `[`, 1),
                             TRUE ~ NA))
...
# Commercial 
unmatched_fbslb_side <- anti_join(bind_rows(fb_presences_country %>% mutate(source = "fishbase"), slb_presences_country %>% mutate(source = "sealifebase")), scinames_excluding_tradeflows %>% ungroup() %>% filter(str_detect(sciname_joinable_to_fishbase, " ")) %>% group_by(sciname_joinable_to_fishbase) %>% distinct(sciname_joinable_to_fishbase), by = c("ScientificName" = "sciname_joinable_to_fishbase")) %>%
...
for (i in unique(pa_only_artis$eez_iso3c)) {
  ...
  unlist() %>%
  unique(), collapse = " "), "[A-Z]", ""), presence_or_absence_all_taxa = case_when(presence_or_absence == 1 ~ 1,
                                                                                    str_detect(unique_vals, sciname_hs_modified) ~ 1, 
                                                                                    sciname_hs_modified %in%
                                                                                    unmatched_species ~ NA,
                                                                                    TRUE ~ 0))
...
fb_slb_commercial_interests <- bind_rows(fb_presences_country %>%
  filter((Status %in% c("native", "Native", "introduced", "endemic") & !is.na(country)) & (Importance %in% c("minor commercial", "highly commercial", "commercial", "subsistence fisheries", "of potential interest"))),
slb_presences_country %>%
  filter((Status %in% c("native", "Native", "introduced", "endemic") & !is.na(country)) & (Importance %in% c("minor commercial", "highly commercial", "commercial", "subsistence fisheries", "of potential interest"))))

# Step 1: Expand found countries for excluded ARTIS species into long format (current state is that countries are a string, where there should only be one country found per column)

# Step 2: Filter out countries that aren't in ARTIS
pa_excluding_artis <- pa_excluding_artis %>%
  separate_rows(countries_found, sep = " ") %>%
  filter(countries_found %in% unique(pa_only_artis$eez_iso3c))
  
# Step 3: Convert fb/slb commercial marine country names to iso3c codes
fb_slb_commercial_interests <- fb_slb_commercial_interests %>%
  select(country, Importance, Species, TrueSpecies, Genus) %>%
...
for (i in unique(pa_only_artis$eez_iso3c)) {
  ...
  unlist() %>%
  unique(), collapse = " "), "[A-Z]", ""), presence_or_absence_all_taxa = case_when(str_detect(unique_vals, sciname) ~ 1,
sciname %in% unmatched_species ~ NA,
TRUE ~ 0))
...
# Create maps
prop_map <- create_map(linked_unlinked_props) +
...
ggsave("../images/prop_map.jpg", plot = prop_map, device = "jpeg", width = 6, height = 4)
```
