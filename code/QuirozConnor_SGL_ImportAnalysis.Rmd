...
taxa_presence_or_absence <- function(data = "fishbase", obtain_stats = FALSE) {
  ...
  if (obtain_stats == TRUE) {
    ...
    fishbase_country_presences <- artis_fishbase_locations %>%
      ...
      mutate(eez_name = countrycode(country, origin = 'country.name', destination = 'iso3c')) %>%
      group_by(Species) %>%
      summarize(countries_found = paste0(unique(eez_name), collapse = " ", Importance = Importance)) %>%
      rename(sciname_joinable_to_fishbase = "Species")
...
fishbase_presences_country %>%
  filter(str_detect(sciname_joinable_to_fishbase, " ")) %>%
  count(joined)


# Need to figure our why data isnt matching up, particularly why including 
pa_excluding_artis <- merged_presence_absence_data_country %>%
  filter(str_detect(sciname_joinable_to_fishbase, " ") & (joined == "only in fishbase" | joined == "only in sealifebase"))
...
pa_excluding_artis %>%
  left_join(bind_rows(fb_presences_country, slb_presences_country) %>% rename(sciname_joinable_to_fishbase = "Species"), by = "sciname_joinable_to_fishbase")
country()
