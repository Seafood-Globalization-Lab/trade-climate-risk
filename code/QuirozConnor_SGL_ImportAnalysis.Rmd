...
taxa_presence_or_absence <- function(data = "fishbase", obtain_stats = FALSE) {
  ...
  if (obtain_stats == TRUE) {
    ...
    consumption_countries_joined <- full_join(consumption_sciname, fishbase_country_presences, by = "sciname_joinable_to_fishbase")
...
unmatched_species <- anti_join(consumption_sciname %>% ungroup() %>% filter(str_detect(sciname_joinable_to_fishbase, " ")), bind_rows(fb_presences_country, slb_presences_country), by = c("sciname_joinable_to_fishbase" = "Species")) %>%
...
for (i in unique(pa_only_artis$eez_iso3c)) {
  ...
  unique(), collapse = " "), "[A-Z]", ""), presence_or_absence_all_taxa = case_when(str_detect(unique_vals, sciname) ~ 1, 
                                                                                    sciname_joinable_to_fishbase %in%
...
# Exploring proportion of absent data
...
# Code my brother put in lol
michael <- 'the best brother' 
 
sean <- 'alright I guess'

print(c('michael is', michael, 'and sean is',sean ))
...
for (i in unique(pa_only_artis$eez_iso3c)) {
  ...
  unlist() %>%
  unique(), collapse = " "), "[A-Z]", ""), presence_or_absence_all_taxa = case_when(str_detect(unique_vals, sciname) ~ 1,
sciname_joinable_to_fishbase %in% unmatched_species ~ NA,
TRUE ~ 0))
