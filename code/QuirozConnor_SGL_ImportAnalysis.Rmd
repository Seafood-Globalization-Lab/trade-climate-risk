...
# faoareas(server = "fishbase")
# faoareas(species_list = c(unique(species_names$taxon_name)))
# species()
...
# 1320 species with sciname_hs_modified variable (This seems more right) - less than 
...
for (i in 1:nrow(artis_fishbase_locations)) {
  for (j in 1:nrow(eez_extents)) {
    ...
    if (lat_check & long_check) {
      ...
      # eez_assignments[i] <- append(eez_assignments, eez_extents$ISO_TER1[j])
      
      # eez_assignments[i] <- c(eez_assignments[i], eez_extents$ISO_TER1[j])
