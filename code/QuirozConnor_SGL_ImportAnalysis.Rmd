...
commercial_products <- inner_join(pa_excluding_artis, fb_slb_commercial_interests, by = c("sciname_joinable_to_fishbase" = "Species", "countries_found" = "country")) %>%
  ...
  select(-c(genus, subfamily, family, order, class, superclass, phylum, kingdom)) %>%
...
# New script to refer to to obtain rgbif taxa data - refer to "accepted" column to filter for target species that we will be getting higher taxa names for.

# rgbif function
name_lookup("Abramis brama")

# Filter to get top ranked species will look something like this: 
commercial_products %>%
  mutate(genus = sapply(sciname_joinable_to_fishbase, function(x) {
    name_lookup(x)[[2]] %>%
      count(genus) %>%
      arrange(genus) %>%
      drop_na() %>%
