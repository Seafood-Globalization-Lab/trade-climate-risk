...
## SWITCH OVER TO 98_Testing_Future_catch_Disaggregation
...
sp_change_ave <- df1 %>%
  group_by(sciname) %>%
  # if we gapfill along these lines, we will probably want to weight by
  # catch or pull from nearby EEZs
  summarise(ssp126_ave = mean(ssp126),
            ssp585_ave = mean(ssp585))
# 
# # Join future data on consumption data
consumption_future_joined <- consumption_future %>%
  full_join(df1, by = c("sciname_hs_modified" = "sciname", "eez_iso3c")) %>%
  full_join(sp_change_ave,
            by = c("sciname_hs_modified" = "sciname")) %>%
  mutate(ssp126 = case_when(
    is.na(ssp126) ~ ssp126_ave,
    TRUE ~ ssp126),
    ssp585 = case_when(
      is.na(ssp585) ~ ssp585_ave,
      TRUE ~ ssp585))


# Join future data on consumption data
consumption_future <- left_join(consumption_future_joined, df1, by = c("sciname_hs_modified" = "sciname", "eez_iso3c"))

# Pivot dataset to longer to make multiscenario visualization easier
consumption_future <- consumption_future_joined %>%
...
e_s_ac_data <- left_join(gdp_clean, trade_gdp_clean) %>%
  ...
  right_join(consumer_stock_change %>% # Add in exposure data
               select(consumer_iso3c, scenario, pct_change), by = "consumer_iso3c") %>%
