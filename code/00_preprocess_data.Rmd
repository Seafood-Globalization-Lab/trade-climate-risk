...
df1 <- df %>%
  ...
  summarize(`2030ssp126` = mean(`2030ssp126`),
            `2030ssp585` = mean(`2030ssp585`),
            `2050ssp126` = mean(`2050ssp126`),
            `2050ssp585` = mean(`2050ssp585`),
            `2100ssp126` = mean(`2100ssp126`),
            `2100ssp585` = mean(`2100ssp585`),
            )
...
consumption_future <- consumption_future %>%
  pivot_longer(cols = c(`2030ssp126`,
                        `2030ssp585`,
                        `2050ssp126`,
                        `2050ssp585`,
                        `2100ssp126`,
...
fao_food <- read_csv("../data/sensitivity/FAOSTAT_data_en_2-26-2025.csv")
fao_food <- fao_food %>%
   mutate(iso3c = countrycode(Area, origin = 'country.name', destination = 'iso3c'))


# Obtain marine foods
marine_foods <- fao_food %>%
  filter(Year == 2019) %>%
  distinct(Item) %>%
  slice(13:23) %>%
  pull(Item)

# Calculate countries' total animal sourced imports
aquatic_animal_imports <- fao_food %>% filter(Element == "Protein supply quantity (t)",
                    Item %in% marine_foods) %>%
  group_by(iso3c, Year) %>%
  summarize(total_aquatic_animal_imports_t = sum(Value) * 1000)

# Calculate countries' total non-animal sourced import supply
total_imports <- fao_food %>% filter(Element == "Protein supply quantity (t)",
                    !Item %in% marine_foods) %>%
  group_by(Year, iso3c) %>%
  summarize(total_imports_t = sum(Value))

# Calculate proportion of countries imports that are aquatic animal proteins
fao_prop_imports <- left_join(aquatic_animal_imports, total_imports, by = c("iso3c", "Year")) %>%
  group_by(iso3c, Year) %>%
  summarize(prop_imports_aquatic_animal = round(total_aquatic_animal_imports_t / (total_imports_t + total_aquatic_animal_imports_t), 3)) %>% 
  exploreARTIS::add_region("iso3c", region.col.name = "region")

# Read in full consumtpiton data (not just marine capture)
consumption_full <- read_csv("../data/full consumption/consumption/consumption_midpoint_HS96_2019.csv")

# Create a list of years for which you have files
years <- 2010:2019
consumption_full <- data.frame()

# Use a loop to read the files and bind them together
consumption_data <- for (i in years) {
...
habitat_method_props <- consumption_full %>%
  ...
  group_by(year, consumer_iso3c, habitat, method) %>%
  summarize(prop_catch = round(total_group_catch / total_catch, 3)) %>%
  ungroup()

# Join artis and fao food balance sheet data (combines habitat/method props to prop animal protein imports byt fao)
artis_fao_fbb <- left_join(habitat_method_props, fao_prop_imports, by = c("consumer_iso3c" = "iso3c", "year" = "Year"))
...
supply_importance <- artis_fao_fbb %>%
  ...
  group_by(year, consumer_iso3c) %>%
  mutate(importance_on_supply = prop_catch * prop_imports_aquatic_animal) %>%
  filter(!is.na(region))

# Visualziation for importance on supply
supply_importance %>%
  filter(year == 2019) %>%
  mutate(importance_on_supply = importance_on_supply * 100) %>%
  ggplot(aes(x = region, y = importance_on_supply, fill = region)) +
...
supply_importance %>%
  ...
  summarize(mean_importance = mean(importance_on_supply)) %>%
  mutate(year = as.Date(paste0(year, "-01-01")),
         mean_importance = mean_importance * 100) %>%
...
left_join(consumption %>%
  ...
  consumption[,c("consumer_iso3c", "hdi", "foreign_dependency")]) %>%
    distinct(consumer_iso3c, .keep_all = TRUE) %>%
  rename(eez_iso3c = "consumer_iso3c") %>%
  filter(!is.na(foreign_dependency)) %>%
  create_map(fill = "hdi") +
...
consumption %>%
  filter(consumer_iso3c != producer_iso3c) %>%
  group_by(consumer_iso3c) %>%
  summarize(total_consumption = sum(consumption_t)) %>%
  rename(eez_iso3c = "consumer_iso3c") %>%
  create_map(fill = "total_consumption") %>%
  labs(fill = "Total imports")
```
...
mora_etal_data <- read_csv("../data/adaptive capacity/learning/global_srcdata_mora2009.csv")

scientific_robustness_clean <- mora_etal_data %>%
  filter(COMPONENT == "scientific-robustness") %>%
  filter(!is.na(VALUE))

h <- country_coverage(scientific_robustness_clean, join_by = "ISO3166")
...
policy_transparency_clean <- mora_etal_data %>%
  filter(COMPONENT == "policy-transparency") %>%
  filter(!is.na(VALUE))

j <- country_coverage(policy_transparency_clean, join_by = "ISO3166")

# capability to implement regulations
mora_etal_data <- read_csv("../data/adaptive capacity/learning/global_srcdata_mora2009.csv")

implementation_capability_clean <- mora_etal_data %>%
  filter(COMPONENT == "implementation-capability") %>%
  filter(!is.na(VALUE))

k <- country_coverage(implementation_capability_clean, join_by = "ISO3166")

# reliance on subsidies
mora_etal_data <- read_csv("../data/adaptive capacity/learning/global_srcdata_mora2009.csv")

subsidies_clean <- mora_etal_data %>%
  filter(COMPONENT == "subsidies") %>%
  filter(!is.na(VALUE))

l <- country_coverage(subsidies_clean, join_by = "ISO3166")

# Count total number of ARTIS countries missing across adaptive capacity sources
length(unique(c(a,b,c,d,e,f,g,h,i,j,k,l)))
missing_artis_ac_countries <- unique(c(a,b,c,d,e,f,g,h,i,j,k,l))

missing_artis_ac_countries <- data.frame(consumer_iso3c = c(a,b,c,d,e,f,g,h,i,j,k,l)) %>%
