...
consumer_stock_change <- consumption_future %>% 
  ...
  summarize(future_weight_t = live_weight_t * future_change_catch,
            change_in_stock = (future_weight_t / live_weight_t)) %>%
  group_by(consumer_iso3c, scenario) %>%
  summarize(change_in_stock = (mean(change_in_stock) - 1) * 100) %>% # convert to percent
  exploreARTIS::add_region("consumer_iso3c", region.col.name = "region") %>%
  filter(!is.na(region))

# Scaled by how important is a stock (i.e., % of a country's total weight)
consumer_stock_change <- consumption_future %>% 
  filter(consumer_iso3c != producer_iso3c,
         !is.na(live_weight_t),
         !is.na(future_change_catch)) %>%
  ...
  mutate(pct_influence = live_weight_t / total_present_weight,
         change_in_stock = pct_influence * 5) %>%
  group_by(consumer_iso3c, scenario) %>%
  summarize(change_in_stock = (mean(change_in_stock) - 1) * 100) %>% # convert to percent
  exploreARTIS::add_region("consumer_iso3c", region.col.name = "region") %>%
  filter(!is.na(region))
...
safer_prospective_products <- consumption_future %>% 
  filter(consumer_iso3c != producer_iso3c,
         !is.na(live_weight_t),
         !is.na(future_change_catch)) %>%
  mutate(future_change_catch = future_change_catch / 100) %>%
  group_by(consumer_iso3c, scenario) %>%
  mutate(weight_future = live_weight_t * future_change_catch,
         total_weight_future = sum(weight_future),
         total_live_weight = sum(live_weight_t),
         change_in_stock = round(total_weight_future / total_live_weight, 3)) %>%
  slice_max(change_in_stock, n = 1) %>%
  count(sciname_hs_modified) %>%
  slice_max(n, n = 1) %>%
  exploreARTIS::add_region("consumer_iso3c", region.col.name = "region") %>%
  group_by(region, sciname_hs_modified, scenario) %>%
  count(sciname_hs_modified) %>%
  filter(!is.na(region))
...
consumption_future %>% 
  ...
  group_by(consumer_iso3c, scenario) %>%
  mutate(weight_future = live_weight_t * future_change_catch,
         total_weight = sum(weight_future, na.rm = TRUE),
         change_in_stock = round(weight_future / total_weight, 3)) %>%
  summarize(change_in_stock = sum(change_in_stock) - 1) %>%
  exploreARTIS::add_region("consumer_iso3c", region.col.name = "region") %>%
  filter(!is.na(region))
...
consumption_future %>% 
  ...
  mutate(weight_future = live_weight_t * future_change_catch,
         ...
         change_in_stock = round(total_weight_future / total_live_weight, 3)) %>%
  group_by(consumer_iso3c, scenario) %>% 
  summarize(change_in_stock = mean(change_in_stock) - 1) %>%
  exploreARTIS::add_region("consumer_iso3c", region.col.name = "region") %>%
  filter(!is.na(region), scenario == "2100ssp585") %>%
...
ungroup() %>%
  mutate(change_in_stock = (change_in_stock - min(change_in_stock)) / 
                              (max(change_in_stock) - min(change_in_stock)))
```
