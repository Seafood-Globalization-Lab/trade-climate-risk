...
length(unique(c(a,b,c,d,e,f,g,h,i,j)))

# assign missing countries to vector
missing_artis_ac_countries <- unique(c(a,b,c,d,e,f,g,h,i,j,k))
...
variable_coverage <- consuming_countries %>%
  ...
  mutate(a = case_when(consumer_iso3c %in% a ~ 1, TRUE ~ 0),
         ...
         g = case_when(consumer_iso3c %in% g ~ 1, TRUE ~ 0),
  ...
  pivot_longer(cols = c(a,b,c,d,e,f,g,h,i,j,k)) %>%
  ...
  mutate(ac_var_name = case_when(
    ...
    ac_var_name == "g" ~ "Mean years schooling",
...
missing_artis_ac_countries <- data.frame(consumer_iso3c = c(a,b,c,d,e,f,g,h,i,j,k)) %>%
...
adaptive_capacity_calcs <- joined_data %>%
  mutate(across(c(gdp, gdp_trade, sanitation, 
                  supermarkets, life_expectancy, prop_labor,
                  avg_yrs_educ, hci, gov_effectiveness,
  ...
  mutate(gdp_scaled = gdp_scaled * (1/3), # Weight each AC variable within AC component (will add to 4 for country if each value is it's max value)
         ...
         avg_yrs_educ_scaled = avg_yrs_educ_scaled * (1/2),
         hci_scaled = hci * (1/2),
...
risk <- left_join(joined_data, adaptive_capacity_calcs, by = "consumer_iso3c") %>%
  filter(!is.na(adaptive_capacity),
         scenario == "2030ssp126") %>%
