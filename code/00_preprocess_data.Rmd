...
unsafe_prospective_products <- consumption_future %>% 
  ...
  filter(!is.na(region))
```
```{r adaptive capacity data}
# Read in GDP data
gdp <- read_csv("../data/adaptive capacity/gdp.csv")
gdp[,c("Country Code","2019")]

# Get ARTIS consuming countries
consuming_countries <- consumption %>%
  filter(consumer_iso3c != producer_iso3c) %>%
  distinct(consumer_iso3c) %>%
  mutate(artis = 1)

# Determine which artis consuming countries are in X adaptive capacity data source
full_join(consuming_countries, gdp[,c("Country Code","2019")] %>% 
            mutate(gdp_coverage = 1), by = c("consumer_iso3c" = "Country Code")) %>%
